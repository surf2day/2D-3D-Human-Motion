...
def bone_loss(prev_sequence, next_sequence, body):
    ...
    for skeleton_index in range(num_of_skeletons):
        for bone in body.bones:
            ...
            bone_length_ref = tf.sqrt(tf.math.squared_difference(x2, x1) + tf.math.squared_difference(y2, y1) + tf.math.squared_difference(z2, z1))
            ...
            bone_length = tf.sqrt(tf.math.squared_difference(x2, x1) + tf.math.squared_difference(y2, y1) + tf.math.squared_difference(z2, z1))

            loss += tf.reduce_sum(input_tensor=tf.abs(bone_length_ref - bone_length) / (bone_length_ref + 0.00001))
    return loss
...
def attention(inputs, 
    ...
    tf.compat.v1.disable_resource_variables() #WARNING: tf.get_variable requires manual check. 
