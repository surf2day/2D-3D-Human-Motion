...
class Sensor(object):
    ...
    #this converts a 2d pose to the world map (my addition)
    def project_2d_to_2d(self, pose2d, subject_id=0, camera_index=0):
        pose2dd = np.empty(shape=(pose2d.shape[0], 2), dtype=np.float32)
        for i in range(pose2d.shape[0]):
            x, y = self._map2d_world_to_depth(pose2d[i, 0], pose2d[i, 1])
            pose2dd[i, 0] = x
            pose2dd[i, 1] = y
        return pose2dd
    ...
    #this converts a 2d pose to the world map (my addition)
    def _map2d_world_to_depth(self, x, y):
        ''' Map from word coordinate to 2D depth coordinate, based on OpenNI '''
#        depth_x = (self._width/self._xz_factor) * (x/z) + self._width/2
#        depth_y = self._height/2 - (self._height/self._yz_factor) * (y/z)
        
        depth_x = (self._width/self._xz_factor) * (x) + self._width/2
        depth_y = self._height/2 - (self._height/self._yz_factor) * (y)
