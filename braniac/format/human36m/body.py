...
class BodyFileReader(object):
    ...
    def __init__(self, file_path):
        ...
        self._frameIndex = 0 #added for sequential reading of sequences, added by cb
...
#        if os.path.splitext(path)[1] == '.h5':
        if os.path.splitext(path)[1] == '.cdf':
#            with h5py.File(path, 'r') as h5f:
            with cdflib.CDF(path) as hf:
#                poses = h5f['3D_positions'][:].T
#                poses = hf['Pose'].reshape(-1,96) 
                poses = hf['Pose'].squeeze() 
