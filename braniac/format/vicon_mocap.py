...
class Sensor(object):
    ...
    def project_2d_to_2d(self, pose3d, subject_id, camera_index=0):
        return self._cameras.project_3d_to_2d(pose3d, subject_id, camera_index)
...
class H36MCameras(object):
    ...
    def __init__(self, file_path):
        '''
        Load camera properties from h5 file or XML
        '''
        splitUp = os.path.splitext(file_path)
        self._camera_count = 4
        
        if splitUp[1] == '.xml':
            self._cams = self._load_camerasXML(file_path)
        else:
...
#below are mods from https://github.com/una-dinosauria/3d-pose-baseline to get camera parameters from xml file

    def _load_camera_paramsXML(self, w0, subject, camera):
        """Load h36m camera parameters
    ...
    def _load_camerasXML(self, bpath, subjects=[1,5,6,7,8,9,11]):
        """Loads the cameras of h36m
        ...
        for s in subjects:
            for c in range(4): # There are 4 cameras in human3.6m
                rcams[(s, c+1)] = self._load_camera_paramsXML(w0, s, c+1)
