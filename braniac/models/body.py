...
class NNResidualDiscriminator(object):
    ...
    def _build_residual_unit(self, inputs, residual_index):
        ...
        '''
        net1 = tf.compat.v1.layers.dense(inputs=inputs,
                               units=self._num_neurons,
                               activation=self._activation, 
                               name="resnet1_{}".format(residual_index+1), 
                               reuse=self._reuse)
        net2 = tf.compat.v1.layers.dense(inputs=net1,
    ...
    def _build(self, inputs):
        ...
        with tf.compat.v1.variable_scope(self._scope + self.__class__.__name__, reuse=self._reuse) as vs:
            ...
            self._output = tf.compat.v1.layers.dense(inputs=net, 
                                           units=self._output_dims, 
                                           name="fc2", 
                                           reuse=self._reuse)
            self._prob = tf.nn.sigmoid(self._output)
...
class RNNGenerator(object):
    ...
    def _build(self, inputs):
        ...
        with tf.compat.v1.variable_scope(self.__class__.__name__, reuse=self._reuse) as vs:
            ...
            Wo = tf.compat.v1.get_variable("Wo", initializer=tf.random.truncated_normal([num_neurons, self._element_shape[0]], stddev=self._stddev))
