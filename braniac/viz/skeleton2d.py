...
class Skeleton2D(object):
    ...
    def draw2d_to_file(self, multiple_sequence_of_skeleton, subject_id, image_path):
        '''
        Render multiple sequences of skeletons into 2D image.
        ...
        Args:
            ...
            image_path(str): path to the output image file.
        '''
        self.draw2d_to_image(multiple_sequence_of_skeleton, subject_id).save(image_path)
    ...
    def draw_to_video_file(self, sequence_of_skeletons, video_path):
        ...
        images = self.draw_to_images_1(sequence_of_skeletons)
    ...
    # my version for drawing sequences that are already 
    def draw2d_to_image(self, multiple_sequence_of_skeleton, subject_id):
        '''
        Render multiple sequences of skeletons into 2D image.
        ...
        for sequence_of_skeleton in multiple_sequence_of_skeleton:
            ...
            for skeleton_index in range(num_of_skeletons):
                pose2d = self._sensor.project_2d_to_2d(sequence_of_skeleton[skeleton_index, :, :], subject_id, camera_index)
                pose2d_max = np.maximum(np.amax(pose2d, axis=0), pose2d_max)
                pose2d_min = np.minimum(np.amin(pose2d, axis=0), pose2d_min)
